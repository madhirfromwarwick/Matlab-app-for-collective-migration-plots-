...
for ii = 1:numel(timeframe)
    ...
    for j = dist_intervals{ii}
        ...
        temp_distance_correlation_data = [reshape(distcorrdata((distcorrdata(:,1)==i)&(distcorrdata(:,8)<=j),2:4),[],1),...
            reshape(distcorrdata((distcorrdata(:,1)==i)&(distcorrdata(:,8)<=j),5:7),[],1)];
        [temp_distcorr, temp_pval] = corr(temp_distance_correlation_data(:,1),temp_distance_correlation_data(:,2),'Rows','Complete');
        dist_corr_set = [dist_corr_set, temp_distcorr];
        dist_corr_set_pval = [dist_corr_set_pval, temp_pval];
...
%% saving
save([filename,'-mah.mat'],"timeframe","importdata",...
    "correlationdata","corrval",...
